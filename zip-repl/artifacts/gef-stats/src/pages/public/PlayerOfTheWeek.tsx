...
interface HistoryRound {
  id: number; weekLabel: string; winner: Player | null; closedAt?: string; season?: string;
}
...
export function PlayerOfTheWeek() {
  ...
  const { data: historyData, isLoading: histLoading } = useQuery<HistoryData>({
  ...
  const history = historyData?.rounds ?? [];
  const leaderboard = historyData?.leaderboard ?? [];
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-10 space-y-12">
        ...
        {/* Hall of Champions */}
        {!histLoading && (leaderboard.length > 0 || history.filter(h => h.winner).length > 0) && (
          <div className="space-y-8">
                  ...
                  {leaderboard.map((entry, i) => (
                    ...
                    >
                      <div className="w-6 text-center shrink-0">
                        {i === 0 && <Crown className="w-4 h-4 text-yellow-400 mx-auto" />}
                        {i === 1 && <Medal className="w-4 h-4 text-slate-400 mx-auto" />}
                        {i === 2 && <Medal className="w-4 h-4 text-amber-600 mx-auto" />}
                        {i >= 3 && <span className="text-xs text-muted-foreground font-bold">#{i + 1}</span>}
                      </div>
                      <PlayerAvatar player={entry.player} size="sm" />
                      <div className="min-w-0 flex-1">
                        <div className="text-sm font-bold text-foreground truncate">{entry.player.name}</div>
                        <div className="text-[10px] text-muted-foreground mt-0.5 flex items-center gap-2">
                          ...
                          <span>{entry.nominations} nominated</span>
                        </div>
                      </div>
                      {entry.wins > 0 && (
                        <div className="text-2xl font-black text-yellow-400 tabular-nums shrink-0">
                          {entry.wins}×
                        </div>
                      )}
                    </motion.div>
            ...
            {history.filter(h => h.winner).length > 0 && (
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <Star className="w-4 h-4 text-muted-foreground" />
                  <h3 className="text-base font-black uppercase tracking-wide text-muted-foreground">Round History</h3>
                </div>
                <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                  {history.filter(h => h.winner).map((h, i) => (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="text-xs font-bold text-foreground truncate">{h.winner!.name}</div>
                        <div className="text-[10px] text-muted-foreground truncate">{h.weekLabel}{h.season ? ` · ${h.season}` : ""}</div>
                      </div>
